...
/// The Client ID from the Auth provider.
@property (nonatomic, readonly, copy) NSString * _Nonnull clientID;
/// A prioritzed list of service providers. They will be used in the order they appear in the array.
@property (nonatomic, readonly, copy) NSArray<NSString *> * _Nonnull preferredIDPS;
@end


/// The OAuth2 details to be used for performing an OAuth2 login flow in an app using MapsIndoors.
...
/// The URL to the Authentication Issuer.
...
/// Find out if external authentication is required.
...
@property (nonatomic, readonly, strong) MPLiveUpdateTopic * _Nonnull topic;
...
+ (MPLiveUpdate * _Nullable)liveUpdateWithDictionary:(NSDictionary<NSString *, id> * _Nonnull)dict topic:(NSString * _Nullable)topic SWIFT_WARN_UNUSED_RESULT;
...
SWIFT_CLASS("_TtC11MapsIndoors24MPAvailabilityLiveUpdate")
...
/// A description of the Booking. This will be the description in the associated event in the calendar system configured in the MapsIndoors CMS.
@property (nonatomic, copy) NSString * _Nullable bookingDescription;
/// The id of the Booking to be used for future references.
@property (nonatomic, copy) NSString * _Nullable bookingId;
/// The date and time the Booking will end,
...
/// The Location the Booking occupies.
@property (nonatomic, strong) id <MPLocation> _Nullable location;
/// The id of the Location the Booking occupies.
@property (nonatomic, copy) NSString * _Nullable locationId;
/// The list of participants. The ids in the list are determined by the calendar system configured in the MapsIndoors CMS.
...
/// The title of the event in the calendar system configured in the MapsIndoors CMS.
...
/// Query object to be used with <code>MPBookingService/bookingsUsing(query:)</code>
...
/// The date and time the Booking will end,
@property (nonatomic, copy) NSDate * _Nullable endTime;
/// The Location the Booking will occupy.
@property (nonatomic, strong) id <MPLocation> _Nullable location;
/// The id of the person responsible for the Booking.
@property (nonatomic, copy) NSString * _Nullable organizerId;
/// The date and time the Booking will start,
@property (nonatomic, copy) NSDate * _Nullable startTime;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
/// The angle toward the horizon measured in degrees, with 0 degrees resulting in a top-down view for a two-dimensional map.
@property (nonatomic, readonly) double viewingAngle;
/// The zoom level of the camera.
@property (nonatomic, readonly) float zoom;
/// Get a <code>MPCameraPosition</code> for a particular target and zoom level.
...
/// \param target Coordinates on Earth the camera is positioned at.
///
/// \param zoom The desired zoom level of the camera.
///
///
/// returns:
...
/// Fit the <code>MPCameraPosition/bearing</code> to point towards the first step of a route.
  MPCameraViewFitModeFirstStepAligned = 0,
/// Fit the <code>MPCameraPosition/bearing</code> to point towards north.
  MPCameraViewFitModeNorthAligned = 1,
/// Fit the <code>MPCameraPosition/bearing</code> to point towards the entire route.
...
SWIFT_CLASS("_TtC11MapsIndoors17MPCountLiveUpdate")
...
/// Optionally create your own cluster icon for the SDK to use.
SWIFT_PROTOCOL("_TtP11MapsIndoors19MPCustomClusterIcon_")
@protocol MPCustomClusterIcon
/// Given a cluster of a given type and size, provide an appropriate icon to represent the cluster.
...
///
/// returns:
/// An image representing the cluster. This will replace the default MapsIndoors cluster image.
...
/// The protocol to conform to when building a floor selector for MapsIndoors.
...
/// Optionally create your own info window for the SDK to use.
SWIFT_PROTOCOL("_TtP11MapsIndoors18MPCustomInfoWindow_")
@protocol MPCustomInfoWindow
/// Called when a Location is selected, and if a UIView is returned from this method that UIView will be shown instead of the default MapsIndoors info window.
/// \param location The selected Location
///
///
/// returns:
/// The view that will replace the default MapsIndoors info window
...
/// <code>true</code> if this dataset is currently being synced.
...
/// Protocol to conform to when implementing a directions renderer for MapsIndoors.
/// A default implementation can be accessed and used via <code>MPMapControl/newDirectionsRenderer()</code>.
...
/// Categories filter. List of category keys as in <code>MPLocation/categories</code>. If multiple categories are supplied the filter will OR’ed, meaning if a Location belongs to at least one of the categories it will be considered as a match.
@property (nonatomic, copy) NSArray<NSString *> * _Nonnull categories;
/// Rectangular geographical bounds filter.
@property (nonatomic, strong) MPGeoBounds * _Nullable bounds;
/// List of location ids that is hierarchically considered as a parent of other locations.
...
/// Include otherwise unsearchable locations in your search. Default value is false.
@property (nonatomic) BOOL ignoreSearchableStatus;
/// Include otherwise inactive locations in your search. Default value is false.
@property (nonatomic) BOOL ignoreActiveStatus;
...
@property (nonatomic, copy) NSArray<NSString *> * _Nonnull types;
/// Initialise a MPFilter with all properties.
/// From Swift all values are optional which will set them to the default values.
/// \param categories See <code>categories</code>. Default is an empty array.
///
/// \param bounds See <code>bounds</code>. Default is <code>nil</code>.
///
/// \param parents See <code>parents</code>. Default is an empty array.
///
/// \param take See <code>take</code>. Default is maximum integer value (all).
///
/// \param skip See <code>skip</code>. Default is <code>0</code>.
///
/// \param depth See <code>depth</code>. Default is <code>1</code>.
///
...
/// Initialise a MPFilter with all properties set to default values as described in <code>init(categories:bounds:parents:take:skip:depth:)</code>.
...
/// Filter behavior class that determines how the result of applying a filter should be displayed on the map. Get the default behavior from <code>MPFilterBehavior/default</code>. The default behavior is no camera movement and no infowindow shown.
...
@property (nonatomic) BOOL moveCamera;
/// Whether <code>MPMapControl</code> should show the info window if a filter only contains one Location. The default behavior is no info window is shown (<code>false</code>).
@property (nonatomic) BOOL showInfoWindow;
@property (nonatomic) BOOL allowFloorChange;
@property (nonatomic) NSInteger animationDuration;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
...
SWIFT_CLASS("_TtC11MapsIndoors10MPHumidity")
@interface MPHumidity : NSDimension
...
SWIFT_CLASS("_TtC11MapsIndoors20MPHumidityLiveUpdate")
...
/// Protocol to conform to when implementing an image provider for MapsIndoors.
/// A default implementation can be accessed and used via <code>MapsIndoorsShared/imageProvider</code>.
...
SWIFT_CLASS("_TtC11MapsIndoors16MPLiveDomainType")
@interface MPLiveDomainType : NSObject
/// Availability Domain Type
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSString * _Nonnull availability;)
+ (NSString * _Nonnull)availability SWIFT_WARN_UNUSED_RESULT;
/// CO2 Domain Type
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSString * _Nonnull co2;)
+ (NSString * _Nonnull)co2 SWIFT_WARN_UNUSED_RESULT;
/// Count Domain Type
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSString * _Nonnull count;)
+ (NSString * _Nonnull)count SWIFT_WARN_UNUSED_RESULT;
/// Humidity Domain Type
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSString * _Nonnull humidity;)
+ (NSString * _Nonnull)humidity SWIFT_WARN_UNUSED_RESULT;
/// Occupancy Domain Type
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSString * _Nonnull occupancy;)
+ (NSString * _Nonnull)occupancy SWIFT_WARN_UNUSED_RESULT;
...
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, copy) NSString * _Nonnull temperature;)
+ (NSString * _Nonnull)temperature SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init OBJC_DESIGNATED_INITIALIZER;
@end
...
SWIFT_CLASS("_TtC11MapsIndoors17MPLiveUpdateTopic")
...
@property (nonatomic, readonly, copy) NSString * _Nonnull topicString;
@property (nonatomic, readonly, copy) NSString * _Nonnull topicStringUrlEscaped;
...
- (nonnull instancetype)initWithDatasetId:(NSString * _Nonnull)datasetId OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)initWithVenueId:(NSString * _Nonnull)venueId OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)initWithBuildingId:(NSString * _Nonnull)buildingId OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)initWithFloorId:(NSString * _Nonnull)floorId OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)initWithRoomId:(NSString * _Nonnull)roomId OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)initWithLocationId:(NSString * _Nonnull)locationId OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)initWithDomainType:(NSString * _Nonnull)domainType OBJC_DESIGNATED_INITIALIZER;
- (MPLiveUpdateTopic * _Nonnull)datasetId:(NSString * _Nonnull)datasetId SWIFT_WARN_UNUSED_RESULT;
- (MPLiveUpdateTopic * _Nonnull)venueId:(NSString * _Nonnull)venueId SWIFT_WARN_UNUSED_RESULT;
- (MPLiveUpdateTopic * _Nonnull)buildingId:(NSString * _Nonnull)buildingId SWIFT_WARN_UNUSED_RESULT;
- (MPLiveUpdateTopic * _Nonnull)floorId:(NSString * _Nonnull)floorId SWIFT_WARN_UNUSED_RESULT;
- (MPLiveUpdateTopic * _Nonnull)roomId:(NSString * _Nonnull)roomId SWIFT_WARN_UNUSED_RESULT;
- (MPLiveUpdateTopic * _Nonnull)locationId:(NSString * _Nonnull)locationId SWIFT_WARN_UNUSED_RESULT;
- (MPLiveUpdateTopic * _Nonnull)domainType:(NSString * _Nonnull)domainType SWIFT_WARN_UNUSED_RESULT;
...
SWIFT_CLASS("_TtC11MapsIndoors19MPLiveTopicCriteria")
...
- (nonnull instancetype)initWithLocationId:(NSString * _Nonnull)locationId OBJC_DESIGNATED_INITIALIZER;
- (nonnull instancetype)initWithDomainType:(NSString * _Nonnull)domainType OBJC_DESIGNATED_INITIALIZER;
- (MPLiveTopicCriteria * _Nonnull)datasetId:(NSString * _Nonnull)datasetId SWIFT_WARN_UNUSED_RESULT;
- (MPLiveTopicCriteria * _Nonnull)venueId:(NSString * _Nonnull)venueId SWIFT_WARN_UNUSED_RESULT;
- (MPLiveTopicCriteria * _Nonnull)buildingId:(NSString * _Nonnull)buildingId SWIFT_WARN_UNUSED_RESULT;
- (MPLiveTopicCriteria * _Nonnull)floorId:(NSString * _Nonnull)floorId SWIFT_WARN_UNUSED_RESULT;
- (MPLiveTopicCriteria * _Nonnull)roomId:(NSString * _Nonnull)roomId SWIFT_WARN_UNUSED_RESULT;
...
/// This holds the data for a single location and a marker to display the data on a map.
...
- (id _Nullable)getLiveValueForKey:(NSString * _Nonnull)key domainType:(NSString * _Nonnull)domainType SWIFT_WARN_UNUSED_RESULT;
/// Get a live update based on a known domain type.
/// \param domainType The <code>MPLiveDomainType</code> to get an update for.
///
///
/// returns:
/// The requested update, if present.
- (MPLiveUpdate * _Nullable)getLiveUpdateForDomainType:(NSString * _Nonnull)domainType SWIFT_WARN_UNUSED_RESULT;
...
/// MapsIndoors internal state is fully populated with data.
  MPLocationSourceStatusComplete = 4,
};


SWIFT_PROTOCOL("_TtP11MapsIndoors16MPLocationUpdate_")
@protocol MPLocationUpdate
...
///   </li>
///   <li>
///     locationId: This id is expected to be unique for a given location source, but not necessarily unique across multiple location sources.
///   </li>
///   <li>
...
/// Adds a property to the location update. Some keys will be recognised as a key for a known property on the resulting <code>MPLocation</code>. If they are not recognised or the known property only supports one value, the property will be added as a field on <code>MPLocation.fields</code>
...
/// Add category key. Possible categories can be reviewed in MapsIndoors CMS or programmatically using the <code>MPCategoriesProvider</code>
...
///
/// returns:
/// The MPLocation object
- (id <MPLocation> _Nonnull)location SWIFT_WARN_UNUSED_RESULT;
@end


/// Protocol  to create instance of <code>MPLocationUpdate</code>
SWIFT_PROTOCOL("_TtP11MapsIndoors23MPLocationUpdateFactory_")
@protocol MPLocationUpdateFactory
- (id <MPLocationUpdate> _Nonnull)updateWithLocation:(id <MPLocation> _Nonnull)location SWIFT_WARN_UNUSED_RESULT;
- (id <MPLocationUpdate> _Nonnull)updateWithId:(NSString * _Nonnull)locationId fromSource:(id <MPLocationSource> _Nonnull)locationSource SWIFT_WARN_UNUSED_RESULT;
@end
...
/// The logger used internally by MapsIndoors.
/// The logging uses <a href="https://developer.apple.com/documentation/os/logging/generating_log_messages_from_your_code#3665947">Unified Logging</a>, so the log levels are the ones described there.
...
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, readonly, strong) MPLog * _Nonnull core;)
+ (MPLog * _Nonnull)core SWIFT_WARN_UNUSED_RESULT;
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@end
...
/// The zoom level that MapsIndoors is currently using for display icons etc. on the map.
@property (nonatomic, readonly) float mapsIndoorsZoom;
/// Select a Location.
/// \param location The Location to select.
///
/// \param behavior The <code>MPSelectionBehavior</code> to apply.
///
...
/// \param floorIndex The index of the Floor to select.
///
- (void)selectWithFloorIndex:(NSInteger)floorIndex;
/// Apply a filter to the map. Only show the <code>MPLocation</code>s included in the list.
/// \param locations An array of <code>MPLocation</code> to show on the map.
///
/// \param behavior The <code>MPFilterBehavior</code> to apply.
///
- (void)setFilterWithLocations:(NSArray<id <MPLocation>> * _Nonnull)locations behavior:(MPFilterBehavior * _Nonnull)behavior;
/// Apply a filter to the map. Only show the <code>MPLocations</code> captured by the <code>MPFilter</code>.
/// \param filter The <code>MPFilter</code> to apply for which Locations to show on the map.
///
/// \param behavior The <code>MPFilterBehavior</code> to apply.
///
...
/// \param entity An MPEntity may be an MPVenue, MPBuilding or MPLocation.
///
...
@protocol MPMapControlDelegate
@optional
/// Triggered when the map was tapped at the coordinate passed in to the method.
/// \param coordinate Map coordinate that was tapped.
///
///
/// returns:
...
/// Triggered when the map with a given marker icon was tapped.
/// \param location The <code>MPLocation</code> of the icon marker that was tapped.
///
///
/// returns:
/// Return true to indicate that you will handle the event, and bypass default MapsIndoors SDK behavior. Otherwise return false.
- (BOOL)didTapIconWithLocation:(id <MPLocation> _Nonnull)location SWIFT_WARN_UNUSED_RESULT;
/// Triggered when the info window of a <code>MPLocation</code> was tapped.
/// \param location The <code>MPLocation</code> of the info window that was tapped.
///
///
/// returns:
/// Return true to indicate that you will handle the event, and bypass default MapsIndoors SDK behavior. Otherwise return false.
...
/// \param selectedLocation The newly selected Location.
///
///
/// returns:
/// Return true to indicate that you will handle the event, and bypass default MapsIndoors SDK behavior. Otherwise return false.
...
/// \param selectedVenue The newly selected Venue.
///
///
/// returns:
/// Return true to indicate that you will handle the event, and bypass default MapsIndoors SDK behavior. Otherwise return false.
...
/// \param selectedBuilding The newly selected Building.
///
///
/// returns:
/// Return true to indicate that you will handle the event, and bypass default MapsIndoors SDK behavior. Otherwise return false.
...
/// \param floorIndex The newly selected Floor index.
///
///
/// returns:
/// Return true to indicate that you will handle the event, and bypass default MapsIndoors SDK behavior. Otherwise return false.
...
SWIFT_CLASS("_TtC11MapsIndoors21MPOccupancyLiveUpdate")
@interface MPOccupancyLiveUpdate : MPLiveUpdate
/// The current capacity in terms of maximum number of people for a given Location.
...
SWIFT_CLASS("_TtC11MapsIndoors20MPPositionLiveUpdate")
...
/// From Swift all values are optional which will set them to the default values.
/// \param coordinate See <code>coordinate</code>.
///
/// \param floorIndex See <code>floorIndex</code>. Default is <code>0</code>.
///
/// \param accuracy See <code>accuracy</code>. Default is <code>0</code>.
///
/// \param bearing See <code>bearing</code>. Default is <code>0</code>.
///
...
/// The geographic point of the position.
@property (nonatomic) CLLocationCoordinate2D coordinate;
/// The index of the floor the <code>coordinate</code> is located on.
@property (nonatomic) NSInteger floorIndex;
/// The probability of the position in <code>coordinate</code>. This is a radius in meters. Can be used to generate an approximation circle.
...
/// Get the default behavior from <code>MPSelectionBehavior/default</code>. The default behavior is that the camera moves to display the selected location and the info window is shown.
...
@property (nonatomic) BOOL moveCamera;
/// Whether <code>MPMapControl</code> should show the info window when a  Location is selected. This applies to both selection via user interaction and when selecting a Location with <code>MPMapControl/select(location:behavior:)</code>.
/// The default behavior is to show an info window (<code>true</code>).
@property (nonatomic) BOOL showInfoWindow;
@property (nonatomic) BOOL allowFloorChange;
@property (nonatomic) NSInteger animationDuration;
...
/// <code>MapsIndoorsShared</code> describes the main entry point to the SDK which is accessed via <code>MPMapsIndoors/shared</code>.
/// Access this shared instance to load, reload or close MapsIndoors solutions using an API key, and navigate the MapsIndoors data.
/// Create new MapControl instances to visualize the MapsIndoors data from the shared instance, in an interactive map engine (Google Maps or Mapbox Maps).
...
- (void)loadWithApiKey:(NSString * _Nonnull)apiKey completionHandler:(void (^ _Nonnull)(NSError * _Nullable))completionHandler;
/// Reload an already loaded MapsIndoors solution.
- (void)synchronizeWithCompletionHandler:(void (^ _Nonnull)(NSError * _Nullable))completionHandler;
...
/// Get or set the current list of applied user roles. If you want to apply user roles and wait for the SDK to load, use <code>MapsIndoorsShared/apply(userRoles:)</code> asynchronously.
